...
def test_matmul_forward():
    np.testing.assert_allclose(
        ndl.matmul(
            ndl.Tensor([[4.95, 1.75, 0.25],
                        [4.15, 4.25, 0.3 ],
                        [0.3 , 0.4 , 2.1 ]]),
            ndl.Tensor([[1.35, 2.2 , 1.55],
                        [3.85, 4.8 , 2.6 ],
                        [1.15, 0.85, 4.15]])
        ).numpy(),
...
def gradient_check(f, *args, tol=1e-6, backward=False, **kwargs):
    ...
    # for i in range(len(args)):
    #     diff = computed_grads[i] - numerical_grads[i]
    #     print("norm:", np.linalg.norm(diff))
    #     print("max :", np.abs(diff).max())
    assert error < tol, f"ERROR: {error}; TOL: {tol}"
